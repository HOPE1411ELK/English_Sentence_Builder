import os, sys, time, random
import tkinter as tk
from tkinter import messagebox
import customtkinter as ctk
import pygame
import string

import threading
from gtts import gTTS
import hashlib  
import re
import shutil

start_time = None


# --- ここから貼り付け ---
MASTER_FILE = "master.txt"
ACTIVE_FILE = "active.txt"
ACQUIRED_FILE = "acquired.txt"


def initialize_files():
  if not os.path.exists(ACTIVE_FILE):
    if os.path.exists(MASTER_FILE):
      shutil.copyfile(MASTER_FILE, ACTIVE_FILE)
    else:
      print("Error: master.txt が見つかりません")
      return []
  with open(ACTIVE_FILE, "r", encoding="utf-8") as f:
    return [line.strip() for line in f if line.strip()]


def get_target_time(sentence):
  word_count = len(sentence.split())
  threshold_table = {
      2: 2.40,
      3: 3.10,
      4: 3.80,
      5: 4.60,
      6: 5.50,
      7: 6.40,
      8: 7.40,
      9: 8.50,
      10: 9.60,
      11: 10.80,
      12: 12.00,
      13: 13.30,
      14: 14.60,
      15: 16.00,
      16: 17.40,
      17: 18.90,
      18: 20.40,
      19: 22.00,
      20: 23.60,
  }

  if word_count <= 2:
    return 2.40
  elif word_count in threshold_table:
    return threshold_table[word_count]
  else:
    # 21語以降：20語（23.60s）を起点に、1語あたり +1.60s（認知0.25s + 探索0.90s + 操作0.45s）
    return 23.60 + (word_count - 20) * 1.60


def get_acquired_count():
  if not os.path.exists(ACQUIRED_FILE):
    return 0
  with open(ACQUIRED_FILE, "r", encoding="utf-8") as f:
    return sum(1 for line in f if line.strip())


def get_rank_info(acquired_count):
  ranks = [
      (3650, 13, "English Deity"),
      (3300, 12, "Language Beast"),
      (2850, 11, "Hyper Drive"),  # ★追加
      (2400, 10, "Sonic Fluent"),
      (2000, 9, "Instinct Master"),
      (1500, 8, "Speed Demon"),  # ★追加
      (1100, 7, "Lightning Speaker"),
      (800, 6, "Rapid Fire"),
      (550, 5, "Flash Striker"),  # ★追加
      (350, 4, "Instant Snapper"),
      (150, 3, "Reflex Gunner"),
      (50, 2, "Swift Starter"),
      (0, 1, "Stroller"),
  ]
  for threshold, rank_level, title in ranks:
    if acquired_count >= threshold:
      return {"rank": rank_level, "title": title, "count": acquired_count}
  return {"rank": 1, "title": "Stroller", "count": 0}


# -------------------- util --------------------
def resource_path(relative_path: str) -> str:
    if hasattr(sys, "_MEIPASS"):
        return os.path.join(sys._MEIPASS, relative_path)
    base_path = os.path.dirname(os.path.abspath(__file__))
    return os.path.join(base_path, relative_path)

def norm(s: str) -> str:
    s = s.replace("’", "'").replace("‘", "'").replace("`", "'")
    s = s.replace(".", "")
    s = " ".join(s.split())
    return s.strip().lower()


# -------------------- sounds --------------------
try:
    # buffer を既定より大きくして音声処理を安定させる
    pygame.mixer.init(buffer=4096)
    pygame.mixer.set_num_channels(32)  # 同時再生チャンネル数も増やす（任意）
except pygame.error as e:
    print(f"[AUDIO] mixer init error: {e}")

def _load_wav(path, vol=0.5):
    try:
        snd = pygame.mixer.Sound(path)
        snd.set_volume(vol)
        return snd
    except pygame.error:
        class _Dummy:
            def play(self): pass
            def set_volume(self, v): pass
        return _Dummy()


startsound    = _load_wav(resource_path("startsound.wav"), 0.8)
correctsound  = _load_wav(resource_path("correctsound.wav"), 0.8)
wrongsound    = _load_wav(resource_path("wrongsound.wav"), 0.8)
giveupsound   = _load_wav(resource_path("giveupsound.wav"), 0.8)
gameoversound = _load_wav(resource_path("gameoversound.wav"), 0.8)
gameendsound  = _load_wav(resource_path("gameendsound.wav"), 0.8)
shufflesound  = _load_wav(resource_path("shufflesound.wav"), 0.8)

# --- BGM 候補（曲のロードは再生時に行う） ---
bgm_candidates = [
    resource_path("battlebgm1.mp3"),
    resource_path("battlebgm1.mp3"),
    resource_path("battlebgm1.mp3"),
    resource_path("battlebgm1.mp3"),
    ]

bgm_files = [p for p in bgm_candidates if os.path.exists(p)]
if not bgm_files:
    print("[BGM] mp3 が見つかりません（スキップ）")

# -------------------- data --------------------

import os
import shutil


def load_sentences_from_file(filename):
  # 1. active.txt がまだ無い場合、master.txt からコピーを作成する
  if filename == "active.txt" and not os.path.exists("active.txt"):
    # resource_path 側、または通常カレントフォルダ側の両方で master.txt を探す
    master_path = resource_path("master.txt")

    if not os.path.exists(master_path):
      master_path = "master.txt"

    if os.path.exists(master_path):
      shutil.copyfile(master_path, "active.txt")
    else:
      messagebox.showerror(
          "ERROR",
          "master.txt が見つかりません。\n同じフォルダに master.txt"
          " を置いてください。",
      )
      return []

  # 2. active.txt を読み込む
  try:
    with open(filename, "r", encoding="utf-8") as f:
      lines = [line.strip() for line in f if line.strip()]

    if not lines:
      raise ValueError("File is empty")

   
    return lines

  except (FileNotFoundError, ValueError) as e:
    messagebox.showerror("ERROR", f"File read error: {e}")
    return []


# 読み込み実行
SENTENCE_FILE = "active.txt"
SENTENCES = load_sentences_from_file(SENTENCE_FILE)

if not SENTENCES:
  messagebox.showwarning(
      "WARNING",
      "The sentence list is empty. Please check your master.txt file.",
  )
  sys.exit(0)




# 難易度のデフォルト閾値（実装仕様）
EASY_MAX = 5       # <= 5 語 → easy
BEGINNER_MAX = 8       # <= 8 語 → Beginner
HIGH_BEGINNER_MIN = 8       # >= 8 語 → High_Beginner
HIGH_BEGINNER_MAX = 10       # <= 10 語 → High_Beginner
INTERMEDIATE_MIN = 11  # >= 11 語 → Intermediate
INTERMEDIATE_MAX = 13  # <= 13 語 → Intermediate
ADVANCED_MIN = 14          # >= 14 語 → Advanced

# -------------------- app --------------------
ctk.set_appearance_mode("light")
ctk.set_default_color_theme("green")

BG = "#C4F3E4"
CARD = "#E4F2E7"
TEXT = "#1C1C1C"
PRIMARY = "#3CC07C"
PRIMARY_HOVER = "#31A37D"
RADIUS = 14
PAD = 12

# ------- 一括フォントスケール（ここだけ変えれば大きさが全体調整できます） -------
UI_SCALE = 1.0
def sz(base):  # baseに倍率を掛ける
    return max(10, int(base * UI_SCALE))

class ESBApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("ENGLISH SENTENCE BUILDER")
        self.geometry("1150x800")

        # ① Tkが初期化された後 ← ここが重要
        self.challenge_mode = tk.IntVar(value=0)

        self.configure(fg_color=BG)
        # 追加: クラスの __init__ で
        self._in_tick = False
        # __init__ 冒頭のラベル作成前あたりに
        self.timer_text = tk.StringVar(value="TIME: 15:00")

        # __init__ 内に追加
        self.listening_mode = ctk.BooleanVar(value=False)


        # ---- fonts（主要ラベルはCTkFontで統一） ----
        self.font_title     = ctk.CTkFont(family="Yu Gothic UI", size=sz(20), weight="bold")
        self.font_timer     = ctk.CTkFont(family="Segoe UI",     size=sz(20), weight="bold")
        self.tile_font      = ctk.CTkFont(family="Yu Gothic UI", size=sz(28), weight="bold")  # タイル
        self.font_q_small   = ctk.CTkFont(family="Segoe UI",     size=sz(18))
        self.font_result    = ctk.CTkFont(family="Segoe UI",     size=sz(28), weight="bold")  # 回答欄（大）
        self.font_status    = ctk.CTkFont(family="Segoe UI",     size=sz(18), weight="bold")  # メッセージ
        self.font_correct   = ctk.CTkFont(family="Segoe UI",     size=sz(20))                 # 正解文（大）
        self.font_avg       = ctk.CTkFont(family="Segoe UI",     size=sz(20), weight="bold")  # AVG/正答率（大）
        self.font_score     = ctk.CTkFont(family="Segoe UI",     size=sz(20), weight="bold")  # SCORE（大）
        self.font_count     = ctk.CTkFont(family="Segoe UI",     size=sz(20), weight="bold")  # COUNT（大）
        self.font_button    = ctk.CTkFont(family="Segoe UI",     size=sz(18), weight="bold")
        self.font_section   = ctk.CTkFont(family="Yu Gothic UI", size=sz(16), weight="bold")
        self.font_hint      = ctk.CTkFont(family="Segoe UI",     size=sz(16))

        # ---- state ----
        self.sentences = SENTENCES
        self.sentence = ""
        self.words = []
        self.shuffled = []
        self.selected = []
        self.used = set()
        self.correct_count = 0
        self.question_count = 0
        self.give_up_count = 0
        self.wrong_attempts = 0
        self.solve_times = []
        self.round_start_time = None
        self.total_time_limit = 15 * 60
        self.timer_running = False
        self.timer_job = None
        self.game_has_ended = False
        self.game_start_time = time.time()

        self.show_period = tk.IntVar(value=1)  # 1=表示, 0=非表示

        # FLASH RETENTION用
        self.flash_mode = tk.BooleanVar(value=False)
        self.flash_timer_job = None


        # strike（ミス×3 or GIVE UP）合算。5で終了
        self.strike_count = 0

        # 語数
        self.word_counts = [len(s.split()) for s in self.sentences]

        # 出題順管理
        
        self.last_candidates = None
        self.last_level = None

        self.last_seed = None
        self.current_seed = None
        self.order = []
        self.order_pos = 0

        # BGM ON/OFF
        self.bgm_on = ctk.StringVar(value="off")

        # 出題モード & 難易度
        self.mode_var = ctk.StringVar(value="random")     # "same"/"random"
        self.level_var = ctk.StringVar(value="random")    # "beginner"/"intermediate"/"normal"/"advanced"


        # ---- time-limit state (new_game で使うので先に作る) ----
        self.game_minutes = tk.IntVar(master=self, value=10)  # 既定15分
        self.time_rbs = []
        self.time_left = self.game_minutes.get() * 60
        self.timer_job = None






        # scoring
        self.score = 0
        self.last_gain = 0

        self._build_ui()
        self.new_game(first=True)
        self.protocol("WM_DELETE_WINDOW", self.on_close)

        self.update_progress_ui()  # ← 【ここに追加！】起動時の初期表示を最新にする

        # __init__ の最後あたりで
        self.configure(cursor="arrow")

        # スリープ・最小化復帰対策
        self.bind("<FocusIn>", self.reset_audio)









    def speak_sentence(self, text: str = None):
        """指定された英文（未指定なら現在の問題文）をGoogle TTSで非同期再生（ハッシュキャッシュ対応）"""
        target_text = text if text else getattr(self, "sentence", "")
        if not target_text:
            return

        # 別スレッドで再生処理を実行（GUIのフリーズ防止）
        threading.Thread(
            target=self._play_tts_worker, args=(target_text,), daemon=True
        ).start()





    def get_audio_filepath(self, clean_text: str) -> str:
        """整形済みテキストから12桁ハッシュIDとパスを取得"""
        base_dir = os.path.dirname(os.path.abspath(__file__))
        audio_dir = os.path.join(base_dir, "audio_cache")
        os.makedirs(audio_dir, exist_ok=True)

        # ここで「実際にどこを探しているか」を絶対パスで表示
        print(f"[DEBUG PATH] スクリプト基準の保存先: {audio_dir}")
        print(f"[DEBUG PATH] 現在の作業ディレクトリ(getcwd): {os.getcwd()}")

        # 事前生成スクリプトと全く同じ方法でハッシュ化
        hash_id = hashlib.md5(clean_text.encode('utf-8')).hexdigest()[:12]
        return os.path.join(audio_dir, f"{hash_id}.mp3")


    def _play_tts_worker(self, text: str):
        """別スレッドで動作する音声再生ワーカー"""
        try:
            # 余計な加工はせず、事前生成スクリプトと同じようにトリムのみ行う
            clean_text = text.strip()

            # 綺麗なテキストでハッシュを取得
            audio_file = self.get_audio_filepath(clean_text)

            print(f"[DEBUG] 変換前: {repr(text)}")
            print(f"[DEBUG] 変換後: {repr(clean_text)}")
            print(f"[DEBUG] 探しているファイル: {audio_file}")
            print(f"[DEBUG] ファイルの有無: {os.path.exists(audio_file)}")

            # キャッシュ判定と再生
            if not os.path.exists(audio_file):
                print(f"[gTTS] キャッシュ未発見のため新規生成中: {clean_text}")
                tts = gTTS(text=clean_text, lang="en")
                tts.save(audio_file)
            else:
                print(f"[CACHE] ★キャッシュ成功★ 読み込み中: {os.path.basename(audio_file)}")

            if not pygame.mixer.get_init():
                pygame.mixer.init()

            pygame.mixer.music.load(audio_file)
            pygame.mixer.music.play()

            while pygame.mixer.music.get_busy():
                time.sleep(0.1)

            pygame.mixer.music.unload()

        except Exception as e:
            print(f"TTS Error: {e}")
    

# ---------- audio reset (sleep / focus restore) ----------
    def reset_audio(self, event=None):
        try:
            # いったん完全停止
            try:
                pygame.mixer.music.stop()
            except:
                pass

            pygame.mixer.quit()
            pygame.mixer.init(buffer=4096)
            pygame.mixer.set_num_channels(32)

            # BGM が ON なら再開
            if self.bgm_on.get() == "on" and not self.game_has_ended:
                self.play_bgm(restart=True)

        except Exception as e:
            print("[AUDIO] reset error:", e)





    # ---------- BGM helper ----------
    def play_bgm(self, restart=False):
        if getattr(self, "game_has_ended", False):
            return
        if self.bgm_on.get() != "on" or not bgm_files:
            return
        try:
            if pygame.mixer.music.get_busy():
                if restart:
                    pygame.mixer.music.stop()
                else:
                    return
            track = random.choice(bgm_files)
            pygame.mixer.music.load(track)
            pygame.mixer.music.set_volume(0.2)
            pygame.mixer.music.play(-1)
        except pygame.error as e:
            print(f"[BGM] play error: {e}")

    def stop_bgm(self):
        try:
            pygame.mixer.music.stop()
        except pygame.error:
            pass

    def on_bgm_toggle(self):
        if getattr(self, "game_has_ended", False):
            self.stop_bgm()
            return
        if self.bgm_on.get() == "on":
            self.play_bgm(restart=True)
        else:
            self.stop_bgm()





    # ---------- 難易度フィルタ ----------
    def _candidate_indices_by_level(self):
        level = self.level_var.get()

        if level == "easy":
            return [i for i, wc in enumerate(self.word_counts) if wc <= EASY_MAX]

        elif level == "beginner":
            return [i for i, wc in enumerate(self.word_counts) if wc <= BEGINNER_MAX]
        
        elif level == "high_beginner":
            return [ i for i, wc in enumerate(self.word_counts)
        if HIGH_BEGINNER_MIN <= wc <= HIGH_BEGINNER_MAX]
        
        
        elif level == "intermediate":
            return [ i for i, wc in enumerate(self.word_counts)
        if INTERMEDIATE_MIN <= wc <= INTERMEDIATE_MAX]


        elif level == "advanced":
            return [i for i, wc in enumerate(self.word_counts) if wc >= ADVANCED_MIN]
        else:
            return list(range(len(self.sentences)))

    # ---------- scoring helpers ----------
    def _time_bonus(self, dur):
        if dur <= 2:  return 24
        if dur <= 3:  return 21
        if dur <= 4:  return 18
        if dur <= 5:  return 15
        if dur <= 6:  return 12
        if dur <= 7:  return 10
        if dur <= 8:  return 9
        if dur <= 9:  return 8
        if dur <= 10: return 7
        if dur <= 15: return 6
        if dur <= 20: return 5
        if dur <= 25: return 4
        if dur <= 30: return 3
        if dur <= 35: return 2
        if dur <= 40: return 1

        return 0   # 30秒超を含め、それ以降は0
    

    def _base_points_on_success(self, wrongs):
        return 5 if wrongs == 0 else 3 if wrongs == 1 else 1

    def _update_score_ui(self):
        sign = "+" if self.last_gain >= 0 else ""
        self.score_label.configure(text=f"SCORE: {self.score} ({sign}{self.last_gain} pts)")
        self.strike_label.configure(text=f"COUNT: {self.strike_count}/5")



    def _length_multiplier(self, word_count: int) -> float:
     
        if word_count <= 2:
            return 0.50
        if word_count <= 3:
            return 0.60
        if word_count <= 4:
            return 0.80
        if word_count <= 5:
            return 1.00
        if word_count <= 7:
            return 1.20
        if word_count <= 9:
            return 1.50
        if word_count <= 11:
            return 1.90
        if word_count <= 13:
            return 2.40
        if word_count <= 14:
            return 3.00
        if word_count <= 15:
            return 3.70
        if word_count <= 16:
            return 4.50
        if word_count <= 17:
            return 5.40
        if word_count <= 18:
            return 6.40
        if word_count <= 19:
            return 7.50
        if word_count <= 20:
            return 8.70
        return 8.70


    # ---------- UI ----------
    def _build_ui(self):

# Header
        top = ctk.CTkFrame(self, corner_radius=0, fg_color="#D4ECD9")
        top.pack(fill="x")

        # 1. 左側：タイトル
        ctk.CTkLabel(top, text="ENGLISH SENTENCE BUILDER",
                     font=self.font_title).pack(side="left", padx=16, pady=12)

        # 2. 右端：タイマー表示（最初に pack して一番右に寄せる）
        self.timer_label = ctk.CTkLabel(top, textvariable=self.timer_text, font=self.font_timer)
        self.timer_label.pack(side="right", padx=(8, 16))
        self.timer_default_color = self.timer_label.cget("text_color") or None

        # 3. タイマーの左隣：FLASHモード切替スイッチ
        self.flash_switch = ctk.CTkSwitch(
            top,
            text="FLASH MODE",
            variable=self.flash_mode,
            command=self.on_flash_toggle, # ★ 切り替え時に実行するメソッドを指定！
            
            switch_width=80,     # 切り替えスイッチ部分の幅
            switch_height=35,    # 切り替えスイッチ部分の高さ
            font=self.font_button)  # 必要に応じて大きめのフォントに変更

            
        self.flash_switch.pack(side="right", padx=(8, 16))



        # 4. FLASHスイッチの左隣：LISTENモード切替スイッチ ★追加
        self.listening_switch = ctk.CTkSwitch(
            top,
            text="LISTEN MODE",
            variable=self.listening_mode,
            command=self.on_listening_toggle,
            switch_width=80,
            switch_height=35,
            font=self.font_button 
        )
        self.listening_switch.pack(side="right", padx=(8, 8))



       
        # Body
        body = ctk.CTkFrame(self, fg_color=BG)
        body.pack(fill="both", expand=True, padx=PAD, pady=PAD)

        # Left panel (game area)
        left = ctk.CTkFrame(body, fg_color=CARD, corner_radius=RADIUS)
        left.pack(side="left", fill="both", expand=True, padx=(0, PAD))

       

        # ── ヘッダー用フレーム（横並びにするための透明な枠） ──
        header_row = ctk.CTkFrame(left, fg_color="transparent")
        header_row.pack(fill="x", padx=18, pady=(0, 6))

        # 左側：問題指示ラベル
        self.question_label = ctk.CTkLabel(
            header_row, text="MAKE CORRECT SENTENCES!", font=self.font_q_small, text_color="#2E2E2E"
        )
        self.question_label.pack(side="left")

        # 右側：プログレス表示用ラベル
        self.progress_label = ctk.CTkLabel(
            header_row,
            text="[Novice] Rank 1  |  Remaining: ---  |  Acquired: 0",
            font=ctk.CTkFont(size=18, weight="bold"),
            text_color="#F39C12",
        )
        self.progress_label.pack(side="right")
        
        self.result_label = ctk.CTkLabel(
            left,
            text="HERE IS YOUR CHALLENGE",
            font=self.font_result,
            text_color="#2E2E2E",
            wraplength=1000,     # ← 横幅(px)。ウィンドウ幅に合わせて調整
            justify="left"       # left / center / right
        )



        self.result_label.pack(fill="x", padx=18, pady=(0, 8))

        # Word pool
        self.pool = ctk.CTkFrame(left, fg_color=CARD)
        self.pool.pack(fill="x", padx=18, pady=(4, 4))

        # Buttons row (Answer / Shuffle / Clear)
        btns = ctk.CTkFrame(left, fg_color=CARD)
        btns.pack(fill="x", padx=18, pady=(10, 18))
        self.answer_btn = ctk.CTkButton(
            btns, text="ANSWER", font=self.font_button, width=180, height=56,
            fg_color=PRIMARY, hover_color=PRIMARY_HOVER, command=self.check_answer
        )
        self.answer_btn.pack(side="right")
        ctk.CTkButton(btns, text="SHUFFLE", font=self.font_button, command=self.shuffle_words).pack(side="left", padx=(0, 8))
        ctk.CTkButton(btns, text="CLEAR",   font=self.font_button, command=self.clear_answer).pack(side="left", padx=(0, 8))

# 音声再生ボタンの例
        self.speaker_btn = ctk.CTkButton(
        btns, 
        text="🔊 LISTEN",font=self.font_button,
        width=180, height=56,
        fg_color="#E57D27",
        hover_color="#D0A030",
        command=lambda: self.speak_sentence(),
    )
        self.speaker_btn.pack(pady=5)



        # Status
        self.message_label = ctk.CTkLabel(left, text="", text_color="#C0392B", font=self.font_status)
        self.message_label.pack(anchor="w", padx=18, pady=(0, 4))
        self.correct_sentence_label = ctk.CTkLabel(left, text="", text_color="#1B7A33", font=self.font_correct)
        self.correct_sentence_label.pack(anchor="w", padx=18, pady=(0, 2))

        # 正答率＆平均
        self.avg_time_label = ctk.CTkLabel(
            left, text="CORRECT: 0/0 | AVG: 0.0 sec | 0%", text_color="#1B4F72", font=self.font_avg
        )
        self.avg_time_label.pack(anchor="w", padx=18, pady=(0, 6))

        # スコア表示（合計と直近得点）
        self.score_label = ctk.CTkLabel(
            left, text="SCORE: 0 (+0 pts)", text_color="#2C3E50", font=self.font_score
        )
        self.score_label.pack(anchor="w", padx=18, pady=(0, 6))

        # 失敗カウント表示
        self.strike_label = ctk.CTkLabel(
            left, text="COUNT: 0/5", text_color="#7D3C98", font=self.font_count
        )
        self.strike_label.pack(anchor="w", padx=18, pady=(0, 10))

        self.praise_label = ctk.CTkLabel(
        left, text="", text_color="#1B7A33", font=self.font_status
        )
        self.praise_label.pack(anchor="w", padx=18, pady=(0, 8))



        # Right panel (controls)

       
       
  
        right = ctk.CTkFrame(body, fg_color=CARD, corner_radius=RADIUS, width=320)
        right.pack(side="right", fill="y")

        # --- TIME LIMIT ---
        ctk.CTkLabel(right, text="TIME LIMIT", font=self.font_section).pack(anchor="w", padx=16, pady=(12, 4))

        time_wrap = ctk.CTkFrame(right, fg_color=CARD)
        time_wrap.pack(anchor="w", padx=12, pady=(0, 10))

# 2x2 のグリッドで配置（横幅が狭くても崩れない）
        values = [(5, "5 min"), (10, "10 min"), (15, "15 min"), (20, "20 min")]
        for i, (val, text) in enumerate(values):
            r, c = divmod(i, 2)  # row 0-1, col 0-1
            rb = ctk.CTkRadioButton(
            time_wrap,
            text=text,
            variable=self.game_minutes,
            value=val,
            command=self._on_time_changed
            )
            rb.grid(row=r, column=c, padx=6, pady=4, sticky="w")
            self.time_rbs.append(rb)

# 列幅調整（任意）
        time_wrap.grid_columnconfigure(0, weight=1)
        time_wrap.grid_columnconfigure(1, weight=1)










        ctk.CTkLabel(right, text="CAPITAL LETTER HINT", font=self.font_section).pack(anchor="w", padx=16, pady=(12, 4))

        self.setting_frame = ctk.CTkFrame(right, fg_color=CARD)
        self.setting_frame.pack(anchor="w", padx=12, pady=(0, 10))

        ctk.CTkRadioButton(
            self.setting_frame,
            text="ON",
            variable=self.challenge_mode,
            value=0
        ).pack(side="left", padx=8, pady=2)

        ctk.CTkRadioButton(
            self.setting_frame,
            text="OFF",
            variable=self.challenge_mode,
            value=1
        ).pack(side="left", padx=8, pady=2)





        ctk.CTkLabel(right, text="PERIOD(.)HINT", font=self.font_section).pack(anchor="w", padx=16, pady=(12, 4))

        period_frame = ctk.CTkFrame(right, fg_color=CARD)
        period_frame.pack(anchor="w", padx=12, pady=(0, 10))

        ctk.CTkRadioButton(
            period_frame,
            text="SHOW",
            variable=self.show_period,
            value=1
        ).pack(side="left", padx=8)

        ctk.CTkRadioButton(
            period_frame,
            text="HIDE",
            variable=self.show_period,
            value=0
        ).pack(side="left", padx=8)





        ctk.CTkLabel(right, text="QUESTION STYLE", font=self.font_section).pack(anchor="w", padx=16, pady=(16, 6))
        style_row = ctk.CTkFrame(right, fg_color=CARD)
        style_row.pack(anchor="w", padx=12, pady=(0, 10))
        ctk.CTkRadioButton(style_row, text="SAME SEQUENCE",   variable=self.mode_var, value="same").pack(anchor="w", pady=(0, 8))
        ctk.CTkRadioButton(style_row, text="RANDOM SEQUENCE", variable=self.mode_var, value="random").pack(anchor="w")

        ctk.CTkLabel(right, text="BGM", font=self.font_section).pack(anchor="w", padx=16, pady=(12, 4))
        bgm_row = ctk.CTkFrame(right, fg_color=CARD)
        bgm_row.pack(anchor="w", padx=12, pady=(0, 10))
        ctk.CTkRadioButton(bgm_row, text="ON",  variable=self.bgm_on, value="on",
                           command=self.on_bgm_toggle).pack(side="left", padx=(0, 10))
        ctk.CTkRadioButton(bgm_row, text="OFF", variable=self.bgm_on, value="off",
                           command=self.on_bgm_toggle).pack(side="left")

        # --- LEVEL (Easy を最上段に分離) ---
        ctk.CTkLabel(right, text="LEVEL", font=self.font_section).pack(anchor="w", padx=16, pady=(12, 4))
        level_wrap = ctk.CTkFrame(right, fg_color=CARD)
        level_wrap.pack(anchor="w", padx=12, pady=(0, 10), fill="x")

        # Easy 用フレーム（最上段）
        row_easy = ctk.CTkFrame(level_wrap, fg_color=CARD)
        row_easy.pack(fill="x", pady=2)
        ctk.CTkRadioButton(row_easy, text="Easy", variable=self.level_var, value="easy").pack(side="left", padx=6)
        ctk.CTkRadioButton(row_easy, text= "High_Beginner", variable=self.level_var, value="high_beginner").pack(side="left", padx=6)

        # その下に2段（Beginner〜Advanced）
        row1 = ctk.CTkFrame(level_wrap, fg_color=CARD)
        row1.pack(fill="x", pady=2)
        row2 = ctk.CTkFrame(level_wrap, fg_color=CARD)
        row2.pack(fill="x", pady=2)

        ctk.CTkRadioButton(row1, text="Beginner",     variable=self.level_var, value="beginner").pack(side="left", padx=6)
        ctk.CTkRadioButton(row1, text="Intermediate", variable=self.level_var, value="intermediate").pack(side="left", padx=6)
        ctk.CTkRadioButton(row2, text="Random",       variable=self.level_var, value="random").pack(side="left", padx=6)
        ctk.CTkRadioButton(row2, text="Advanced",         variable=self.level_var, value="advanced").pack(side="left", padx=6)
        

        ctrl = ctk.CTkFrame(right, fg_color=CARD)
        ctrl.pack(fill="x", padx=12, pady=(20, 10))

        # ★ self.giveup_btn に代入
        self.giveup_btn = ctk.CTkButton(
        ctrl, text="GIVE UP", font=self.font_button, command=self.give_up
)
        self.giveup_btn.pack(fill="x", pady=(0, 8))

        # NEW GAME ボタンはそのまま
        ctk.CTkButton(
        ctrl, text="NEW GAME", font=self.font_button, command=self.new_game
).pack(fill="x")

        ctk.CTkLabel(right, text="Music: MaouDamashii").pack(side="bottom", pady=10)


    # ---------- ゲームロジック ----------
    def _build_pool(self):
        # 1. モード判定を先頭で行う
        is_flash = self.flash_mode.get() if hasattr(self, 'flash_mode') else False
        is_listen = self.listening_mode.get() if hasattr(self, 'listening_mode') else False

        # 2. ★チラつき防止：タイルの生成やテキスト更新が入る前に、即座に READY 表示にする
        if is_flash or is_listen:
            if hasattr(self, 'result_label'):
                self.result_label.configure(
                    text="READY... 3", 
                    text_color="#E67E22"
                )

        # ----------------------------------------------------
        # (既存のタイル生成・配置処理: button の destroy や grid 配置など)
        for w in self.pool.winfo_children():
            w.destroy()
        cols = 4
        self.pool_buttons = []
        for i, w in enumerate(self.shuffled):
            b = ctk.CTkButton(
                self.pool, text=w, width=160, height=48, font=self.tile_font,
                corner_radius=RADIUS, fg_color="white", text_color=TEXT,
                hover_color="#EAF7EF",
                command=lambda ww=w, idx=i: self.on_word_click(ww, idx)
            )
            r, c = divmod(i, cols)
            b.grid(row=r, column=c, padx=8, pady=8, sticky="ew")
            self.pool_buttons.append(b)
        for c in range(cols):
            self.pool.grid_columnconfigure(c, weight=1)
        # ----------------------------------------------------

        # 3. タイマー開始または通常表示の呼び出し
        if is_flash or is_listen:
            self.start_flash_timer()
        else:
            self.cancel_flash_timer()
            self.is_tile_locked = False
            self.set_tiles_enabled_style()
            self._update_result()

# ★ ここを追加！ 通常モード時は問題表示完了と同時に計測をスタートする
            import time
            self.round_start_time = time.perf_counter()

       
    def on_word_click(self, word, idx):

# --- 追加：タイマー表示中（ロック中）はクリックを無視する ---
        if getattr(self, 'is_tile_locked', False):
            return

        if idx in self.used:
            return
        self.used.add(idx)
        self.selected.append(word)
        self.pool_buttons[idx].configure(state="disabled", fg_color="#AFAFAF")
        self._update_result()

    
    def _update_result(self):
        # モード状態およびフェーズの取得
        is_flash_on = hasattr(self, 'flash_mode') and self.flash_mode.get()
        is_listen_on = hasattr(self, 'listening_mode') and self.listening_mode.get()
        is_ready = getattr(self, 'is_tile_locked', False) and getattr(self, 'flash_step', '') == "READY"

        # 1. プレイヤーが単語タイルを1つでも選択している場合
        if self.selected:
            words_per_line = 10
            lines = [
                " ".join(self.selected[i:i+words_per_line])
                for i in range(0, len(self.selected), words_per_line)
            ]
            text = "\n".join(lines)
            color = "#2E2E2E"
            if hasattr(self, 'result_label'):
                self.result_label.configure(text=text, text_color=color)

        # 2. まだタイルを一度も選択していない場合
        else:
            # READY カウントダウン中は READY... 表示を崩さず維持
            if is_ready:
                pass
            elif is_listen_on:
                text = "[ LISTEN & RECONSTRUCT! ]"
                color = "#2980B9"
                if hasattr(self, 'result_label'):
                    self.result_label.configure(text=text, text_color=color)
            elif is_flash_on:
                if getattr(self, 'flash_step', '') == "SHOW":
                    # お手本表示フェーズ（英文記憶タイム）
                    display_words = getattr(self, 'words', [])
                    words_per_line = 10
                    lines = [
                        " ".join(display_words[i:i+words_per_line])
                        for i in range(0, len(display_words), words_per_line)
                    ]
                    text = "\n".join(lines) if lines else "HERE IS YOUR CHALLENGE"
                    color = "#2E2E2E"
                else:
                    # 伏字フェーズ
                    text = "[ MEMORIZE & RECONSTRUCT! ]"
                    color = "#C0392B"
                if hasattr(self, 'result_label'):
                    self.result_label.configure(text=text, text_color=color)
            else:
                # 通常モード（FLASH / LISTEN ともに OFF）
                text = "HERE IS YOUR CHALLENGE"
                color = "#2E2E2E"
                if hasattr(self, 'result_label'):
                    self.result_label.configure(text=text, text_color=color)

        # 3. アベレージタイム・スコアの更新（ここを常に実行）
        if hasattr(self, 'avg_time_label'):
            pct = (self.correct_count / self.question_count * 100) if self.question_count else 0.0
            self.avg_time_label.configure(
                text=f"CORRECT: {self.correct_count}/{self.question_count} | AVG: {self._avg_time():.1f} sec | {pct:.0f}%"
            )
   

        def toggle_flash_mode(self):
        # FLASH OFF に切り替わった場合のみ実行する
         if not self.flash_mode.get():
            # 残っているフラッシュタイマーがあれば停止・消去
            if hasattr(self, 'flash_timer_job') and self.flash_timer_job is not None:
                self.after_cancel(self.flash_timer_job)
                self.flash_timer_job = None

            # カウントダウンタイマーも残っていれば停止・消去
            if hasattr(self, 'cd_timer_job') and self.cd_timer_job is not None:
                self.after_cancel(self.cd_timer_job)
                self.cd_timer_job = None

            # 1. タイルの操作ロックを確実に解除（OFF時のみ）
            self.is_tile_locked = False

            # 2. タイマー基準時間を「今」にセット（OFF時のみ）
            self.round_start_time = time.perf_counter()

            # 選択状態をリセットし、画面を通常モード用に再描画
            self.restore_question_text()
            self._update_result()

         else:
            # FLASH ON に切り替わった場合は、次の問題からフラッシュを開始できるよう状態を整える
            self.start_flash_timer()
    

    def _avg_time(self):
        return (sum(self.solve_times) / len(self.solve_times)) if self.solve_times else 0.0

    def shuffle_words(self):
        """単語タイルのシャッフル（ロック中は無効）"""
        # 1. READYカウントダウン中やお探しの表示中（ロック時）はシャッフルを禁止
        if getattr(self, 'is_tile_locked', False):
            return

        # 2. 配列のシャッフルと選択状態のリセット
        random.shuffle(self.shuffled)
        if 'shufflesound' in globals() or hasattr(self, 'shufflesound'):
            try:
                shufflesound.play()
            except Exception:
                pass

        self.selected.clear()
        self.used.clear()

        # 3. ★ _build_pool() を使わず、既存ボタンのテキスト更新と有効化を行う
        if hasattr(self, 'pool_buttons') and self.pool_buttons:
            for i, (btn, w) in enumerate(zip(self.pool_buttons, self.shuffled)):
                btn.configure(
                    text=w,
                    state="normal",
                    fg_color="white",
                    text_color=TEXT,
                    command=lambda ww=w, idx=i: self.on_word_click(ww, idx)
                )

        # 4. 表示とメッセージのリセット
        self._update_result()
        if hasattr(self, 'message_label'):
            self.message_label.configure(text="")


    def clear_answer(self):
        """CLEARボタン押下時の処理（タイマーを再起動せず選択のみリセット）"""
        # カウントダウン中（タイルロック時）は操作をスキップ
        if getattr(self, "is_tile_locked", False):
            return

        # 選択状態をリセット
        self.selected.clear()
        self.used.clear()

        # メッセージ表示のクリア
        if hasattr(self, 'message_label'):
            self.message_label.configure(text="")

        # モードに応じた表示の更新（_build_pool は呼び出さない）
        is_flash = self.flash_mode.get() if hasattr(self, 'flash_mode') else False
        is_listen = self.listening_mode.get() if hasattr(self, 'listening_mode') else False

        if is_listen:
            if hasattr(self, 'result_label'):
                self.result_label.configure(
                    text="[ LISTEN & RECONSTRUCT! ]", 
                    text_color="#2980B9"
                )
        elif is_flash:
            if hasattr(self, 'result_label'):
                self.result_label.configure(
                    text="[ MEMORIZE & RECONSTRUCT! ]", 
                    text_color="#C0392B"
                )
        else:
            self._update_result()

        # タイルの表示状態（押せる状態）を再描画
        self.set_tiles_enabled_style()


    
    def _count_strike_and_next(self, reason_text=""):
        self.strike_count += 1
        if reason_text:
            self.message_label.configure(text=f"{reason_text} → COUNT {self.strike_count}/5", text_color="#C0392B")
        self._update_score_ui()
        if self.strike_count >= 5:
            self.end_game()
        else:
            self.reset_round()

    def _fail_current_round(self):
        self.last_gain = -15
        self.score += -15
        self.question_count += 1
        self.correct_sentence_label.configure(text=f"THE ANSWER IS\n{self.sentence}")
        self._update_score_ui()
        self._count_strike_and_next("MISS x3 → -15 pts")

    
    def get_sound_count(self,elapsed):
        if elapsed < 6.0:
            return 3      # かなり速い
        elif elapsed < 9.0:
            return 2      # 少し速い
        else:
            return 1      # 通常

    def play_correct_sound(self, count):
        for _ in range(count):
          correctsound.play()
          pygame.time.delay(100)

    def normalize_sentence(self, s: str) -> str:
        s = s.lower()
        
        return s


    def _get_answer_for_check(self):
        """
        正解判定用の文を返す
        """
        s = self.sentence

        if self.challenge_mode.get() == 1:  # NO HINTS / Challenge
            s = s.lower()
            
        return s




    def _apply_period_setting(self, sentence: str) -> str:
        s = sentence.rstrip()

        if self.show_period.get() == 0:
            # 非表示：文末の . を除去
            if s.endswith("."):
               s = s[:-1]
        else:
            # 表示：なければ付ける
            if not s.endswith("."):
                s += ""

        return s



    def check_answer(self):

        dur = 0.0  # ← 必須（UnboundLocalError防止）
        user = " ".join(self.selected)
        answer = self._get_answer_for_check()
        pass_status = ""  # ← これを追加しておく

        # ── 正解 ────────────────────────────────
        if norm(user) == norm(answer):

           
            # ★ None チェック ＆ time.time() に変更
            if self.round_start_time is not None:
                solve_time = max(0.0, time.perf_counter() - self.round_start_time)
            else:
                solve_time = 0.0

            dur = solve_time

            self.cancel_flash_timer()
            
            # ── ここを書き換え ─────────────────────────
            # 目標秒数を取得（単語数から計算）
            target_time = get_target_time(" ".join(self.words))
            # 目標秒数以内なら3回、過ぎていれば1回
            count = 3 if solve_time <= target_time else 1
            self.play_correct_sound(count)
# ──────────────────────────────────────────
            
            
            self.correct_count += 1
            self.question_count += 1

            base = self._base_points_on_success(self.wrong_attempts)
            bonus = self._time_bonus(dur)

            wc = len(self.words)
            mult = self._length_multiplier(wc)

            raw = base + bonus
            gained = int(round(raw * mult))

            self.last_gain = gained
            self.score += gained

            self.message_label.configure(
                text=f"+{gained} pts  (base {base}, time +{bonus}, "
                     f"len {wc} ×{mult:.2f}, {dur:.1f}s, wrong {self.wrong_attempts})",
                text_color="#16A085"
            )



            # ▼▼▼【ここから追加：Progress Mode 卒業判定】▼▼▼
            target_time = get_target_time(self.sentence)
            is_graduated = dur <= target_time

            if is_graduated:
              # 1. acquired.txt に追記
              with open("acquired.txt", "a", encoding="utf-8") as f:
                f.write(self.sentence + "\n")

              # 2. active.txt から直接該当の文を削除して上書き保存
              # （メモリ上の self.sentences はいじらないのでインデックスが狂いません）
              if os.path.exists("active.txt"):
                with open("active.txt", "r", encoding="utf-8") as f:
                  lines = [line.strip() for line in f if line.strip()]

                if self.sentence in lines:
                  lines.remove(self.sentence)
                  with open("active.txt", "w", encoding="utf-8") as f:
                    for s in lines:
                      f.write(s + "\n")

              pass_status = f" [ACQUIRED! Target: {target_time:.1f}s]"
            else:
              pass_status = f" [Target: {target_time:.1f}s]"
            # ▲▲▲【ここまで追加】▲▲▲





            self.correct_sentence_label.configure(
                text=f"CORRECT!  Solve Time: {solve_time:.2f} sec\n{self.sentence}"
            )

            self.solve_times.append(dur)
            self.wrong_attempts = 0


            self.reset_round()
            self.restore_question_text()
            self._update_score_ui()
            self.update_progress_ui()  # ← 【ここに追加！】問題が1問減ったので、画面の数字を更新する


            # ★ 1. タイマーを完全に停止・リセット
            self.cancel_flash_timer()

            # ★ 2. 次の新しい英文をセット
            self._pick_sentence()

            # ★ 3. 新しい英文に対してFLASHタイマー（カウントダウン）を開始
            self.start_flash_timer()

            

        # ── 不正解 ──────────────────────────────
        else:
            self.wrong_attempts += 1
            wrongsound.play()
            self.correct_sentence_label.configure(text="")

            if self.wrong_attempts >= 3:

                self.cancel_flash_timer()

                self._fail_current_round()
                self.message_label.configure(text="")
                giveupsound.play()
            else:
                self.shuffle_words()
                self.message_label.configure(
                    text=f"WRONG! TRY AGAIN ({self.wrong_attempts}/3)",
                    text_color="#C0392B"
                )
    
    
            

    def give_up(self):

            self.cancel_flash_timer()
            
            self.message_label.configure(text="", text_color="#5BE1BE")
            self.give_up_count += 1
            giveupsound.play()

            self.last_gain = -20
            self.score += -20
            self._update_score_ui()

            self.correct_sentence_label.configure(
        text=f"THE ANSWER IS\n{self.sentence}"
    )
            self.question_count += 1
            self._count_strike_and_next("GIVE UP → -20 pts")


    def _force_end_by_giveup(self):
         self.stop_timer()
         self.end_game()



    def new_game(self, first=False):
    # --- BGM/フラグ類 ---
        startsound.play()
        self.game_has_ended = False
        self.stop_bgm()
        self.play_bgm(restart=True)

        self.timer_label.configure(text_color=self.timer_default_color)
        self.praise_label.configure(text="")
                                    
    # --- ボタン/カウンタ初期化 ---
        self.answer_btn.configure(state="normal")
        self.giveup_btn.configure(state="normal")   # 再開時に有効化
        self.give_up_count = 0
        self.wrong_attempts = 0
        self.strike_count = 0

        self.correct_sentence_label.configure(text="")
        self.message_label.configure(text="")

        self.correct_count = 0
        self.question_count = 0
        self.solve_times.clear()
        self.score = 0
        self.last_gain = 0
        self._update_score_ui()

        self.avg_time_label.configure(text=f"CORRECT: 0/0 | AVG: 0.0 sec | 0%")


        current_level = self.level_var.get()

        if current_level != self.last_level:
            # レベルが変わったら same sequence をリセット
            self.last_seed = None
            self.last_candidates = None

        self.last_level = current_level




    # --- 出題候補の作成 ---
        candidates = self._candidate_indices_by_level()
        if not candidates:
           messagebox.showwarning("LEVEL FILTER", "No sentence matches the selected level. Back to Normal.")
           self.level_var.set("normal")
           candidates = list(range(len(self.sentences)))

        if self.mode_var.get() == "same" and (self.last_seed is not None):
           self.current_seed = self.last_seed
           candidates = self.last_candidates[:]   # ← 重要
        else:
           self.current_seed = random.randrange(2**31)
           self.last_seed = self.current_seed
           self.last_candidates = candidates[:]   # ← 保存

        rng = random.Random(self.current_seed)
        self.order = candidates[:]
        rng.shuffle(self.order)
        self.order_pos = 0



    # --- 制限時間の適用（★ここが今回の肝） ---
        self._set_time_controls_enabled(False)                # ゲーム中は変更不可
        self.time_left = self.game_minutes.get() * 60         # ラジオ選択分
        self._update_timer_label(self.time_left)
        self.stop_timer()                                     # 念のため二重起動防止
        self.start_timer()                                    # カウントダウン開始

    # --- 1問目のセット ---
        self.reset_round()

        



    
    def reset_round(self):
        self.message_label.configure(text="", text_color="#5BE1BE")
        self.selected.clear()
        self.used.clear()
        self._pick_sentence()
        self.wrong_attempts = 0

        self.round_start_time = time.perf_counter()
    # 重いUI再構築は idle に回す
        self.after_idle(self._rebuild_round)

        # ▼▼ FLASH RETENTION タイマーの開始 ▼▼
        self.restore_question_text()
        self.start_flash_timer()

    def _rebuild_round(self):
        self._build_pool()
        self._update_result()

# --------------------------------------------------
        # ★ 一番最後に追加：もし現在フラッシュでロック中なら、今出来上がったタイルをグレーにする！
        if getattr(self, 'is_tile_locked', False):
            self.set_tiles_disabled_style()
        # --------------------------------------------------




    def _make_challenge_sentence(self, sentence):
        s = sentence.lower()                  # 全部小文字
        
        return s

    
        

    

    def _pick_sentence(self):
        if self.order_pos >= len(self.order):
            self.end_game()
            return

        idx = self.order[self.order_pos]
        self.order_pos += 1

        self.sentence = self.sentences[idx]

        # ① まず表示用文を作る
        display_sentence = self.sentence

        # ② ピリオド設定を反映
        display_sentence = self._apply_period_setting(display_sentence)

        # ③ Challenge モードなら小文字化（表示用だけ）
        if self.challenge_mode.get() == 1:
            display_sentence = display_sentence.lower()

        # ④ 分解
        self.words = display_sentence.split()
        self.shuffled = self.words[:]
        random.shuffle(self.shuffled)

        self.question_label.configure(text="MAKE CORRECT SENTENCES!")

        # ★【修正点】
        # FLASHモードの時は、出題時には round_start_time を記録しない（Noneまたはそのままにする）
        # ※通常モード時のみ、出題時の time.time() を記録する
        if not hasattr(self, 'flash_mode') or not self.flash_mode.get():
            self.round_start_time = time.time()  # perf_counter() ではなく time.time() に変更
        else:
            self.round_start_time = None  # FLASHモード時は伏字(hide_question_text)になった瞬間に記録するため

        
        # ▼【追加】出題直後は result_label に見本英文を表示
        if hasattr(self, 'result_label'):
            self.result_label.configure(text=display_sentence, text_color="#2E2E2E")






    def start_timer(self):
        """タイマー開始（New Game 時に呼ぶ）"""
        self.stop_timer()  # 二重起動防止
        self.timer_job = self.after(1000, self._tick)

    
    def _tick(self):
        """1秒ごとのカウントダウン処理"""
        if self._in_tick:
            # 何かの理由で再入しそうになったらスキップ
            self.timer_job = self.after(1000, self._tick)
            return

        self._in_tick = True
        try:
            if getattr(self, "game_has_ended", False):
                self.stop_timer()
                return

            self.time_left -= 1
            self._update_timer_label(self.time_left)
            
            # 60秒→オレンジ / 10秒→赤 / それ以外→元色
            if self.time_left <= 10:
               self.timer_label.configure(text_color="#DC0B0B")   # 赤
            elif self.time_left <= 60:
               self.timer_label.configure(text_color="#EDB90C")   # オレンジ
            else:
               self.timer_label.configure(text_color=self.timer_default_color)  # 元色

            
            if self.time_left <= 0:
               self.end_game(time_up=True)
               self.stop_timer()
               return

            # 次の tick を予約
            self.timer_job = self.after(1000, self._tick)
        finally:
            self._in_tick = False







    def stop_timer(self):
            """タイマー停止"""
            if self.timer_job is not None:
                try:
                    self.after_cancel(self.timer_job)
                except Exception:
                    pass
            self.timer_job = None

    

    def end_game(self, time_up: bool = False):
    # 重複終了ガード
        if self.game_has_ended:
           return
        self.game_has_ended = True

    # タイマー停止＆次回のために時間設定UIを戻す
        self.stop_timer()
        self._set_time_controls_enabled(True)

    # 乱数シード保存（sameモード再現用）
        self.last_seed = self.current_seed

    # UI・サウンド
        self.answer_btn.configure(state="disabled")
        self.giveup_btn.configure(state="disabled")
        self.stop_bgm()
        


        if time_up:
            gameendsound.play()
            praise = self._final_praise_en(self.score)
            self.correct_sentence_label.configure(
                text=(f"TIME UP!\n"
                  f"{praise}\n"
                  f"SCORE: {self.score}\n"
                  "PRESS NEW GAME TO RESTART.\n"
                  f"THE ANSWER IS\n{self.sentence}")
            )
        else:
            gameoversound.play()
            self.correct_sentence_label.configure(
                text=(f"SCORE: {self.score}\n"
                  "GAME OVER! PRESS NEW GAME TO RESTART.\n"
                  f"THE ANSWER IS\n{self.sentence}")
            )


   
    


    def theend_game2(self):
        if self.game_has_ended:
            return
        self.game_has_ended = True
        self.stop_timer()
        self.last_seed = self.current_seed

        self.correct_sentence_label.configure(
            text=(f"THE ANSWER IS\n{self.sentence}\n"
                  f"SCORE: {self.score}\n"
                  "GAME OVER")
        )
        self.answer_btn.configure(state="disabled")
        self.giveup_btn.configure(state="disabled")   # ★ 追加（時間切れ終了でも無効化）
        self.stop_bgm()




        # === 2) 制御系ユーティリティ ===
    def _set_time_controls_enabled(self, enabled: bool):
        state = "normal" if enabled else "disabled"
        for rb in self.time_rbs:
            rb.configure(state=state)

    def _on_time_changed(self):
    # ラジオを切り替えたとき（ゲーム開始前の想定）
        mins = self.game_minutes.get()
        self.time_left = mins * 60
        self._update_timer_label(self.time_left)

    

    def _update_timer_label(self, secs: int):
        if secs < 0:
           secs = 0
        m, s = divmod(secs, 60)
        self.timer_text.set(f"TIME: {m:02d}:{s:02d}")

    

    def _final_praise_en(self, score: int) -> str:
        """
        Time Up 専用：最終スコアに応じた英語の褒め言葉を返す。
        15分基準で正規化して判定。難易度ラジオボタンに応じて倍率を適用。
        Easy=1.0 / Beginner=1.25 / Intermediate=1.5 / Normal=1.75
        """
        # 制限時間（分）
        try:
            minutes = int(self.game_minutes.get())
        except Exception:
            minutes = 15
        minutes = max(1, minutes)

        # --- 難易度倍率（つづりゆらぎ対応）---
        try:
            lv = str(self.level_var.get()).strip().lower()
        except Exception:
            lv = "random"  # デフォルトをNormalに

        mult_table = {
            "easy": 1.00,
            "beginner": 1.25,
            "high_beginner": 1.375,
            "intermediate": 1.50,
            "random": 1.75,
            "advanced": 2.0,
        }
        mult = mult_table.get(lv, 1.00)  # 未定義は等倍

        # 倍率を掛けてから 15分換算
        adjusted = int(round(score * mult))
        norm_score = int(round(adjusted * 15 / minutes))

        # ランク判定
        
        if norm_score >= 1400:
            options = [
                "RANK: AAA  Legendary focus! Unstoppable!",
                "RANK: AAA  Incredible! You crushed it!",
                "RANK: AAA  Godlike precision! Absolute mastery!",
                "RANK: AAA  Flawless execution! You're in the zone!",
                "RANK: AAA  Mind-blowing speed! Pure perfection!"
            ]
        elif norm_score >= 1100:
            options = [
                "RANK: AA  Outstanding performance!",
                "RANK: AA  Top-tier play!",
                "RANK: AA  Lightning fast! Superb accuracy!",
                "RANK: AA  Phenomenal skill! Almost unbeatable!",
                "RANK: AA  Brilliant run! You're dominating!"
            ]
        elif norm_score >= 800:
            options = [
                "RANK: A  Excellent run! You're on fire!",
                "RANK: A  Great momentum!",
                "RANK: A  Fantastic focus! Keep pushing!",
                "RANK: A  Sharp mind, quick reflexes!",
                "RANK: A  Impressive work! Victory in sight!"
            ]
        elif norm_score >= 500:
            options = [
                "RANK: B  Nice work! Steady and sharp!",
                "RANK: B  Well played!",
                "RANK: B  Strong effort! Finding your rhythm!",
                "RANK: B  Way to go! Getting smoother every time!",
                "RANK: B  Good pacing! Keep the vibe going!"
            ]
        elif norm_score >= 200:
            options = [
                "RANK: C  Good effort! Keep it up!",
                "RANK: C  Solid progress!",
                "RANK: C  Nice warm-up! You're building speed!",
                "RANK: C  Step by step! You're on the right track!",
                "RANK: C  Decent run! Ready for the next challenge?"
            ]
        elif norm_score > 0:
            options = [
                "RANK: D  Solid start! Build on this!",
                "RANK: D  You're getting there!",
                "RANK: D  First step taken! Keep moving forward!",
                "RANK: D  Every master started here. Practice makes perfect!",
                "RANK: D  Good hustle! Focus and try again!"
            ]
        else:
            options = [
                "RANK: F  Tough round—but you've got this next time!",
                "RANK: F  Shake it off and come back stronger!",
                "RANK: F  Don't give up! Reset and reload!",
                "RANK: F  Mistakes are proof you're trying. Go again!",
                "RANK: F  A temporary bump in the road—your comeback starts now!"
            ]

        return random.choice(options)
       




    def _update_praise(self):
        msg, color = self._praise_for_score(self.score)
        self.praise_label.configure(text=msg, text_color=color)

    def on_close(self):
        try:
            self.stop_bgm()
            pygame.mixer.quit()
            pygame.quit()
        except:
            pass
        self.destroy()


# ---------- FLASH RETENTION 制御ロジック ----------
    def on_flash_toggle(self):
        """スイッチ切り替え時の処理"""
        if not self.flash_mode.get():
            self.cancel_flash_timer()
            self.restore_question_text()
        else:
            self.start_flash_timer()


    def set_tiles_disabled_style(self):
        """フラッシュ表示中：すべてのプールタイルをグレーアウト（操作不能）にする"""
        if hasattr(self, 'pool_buttons') and self.pool_buttons:
            for btn in self.pool_buttons:

                hide_color = "#C7C7C8"
                try:
                    btn.configure(
                       
                        state="disabled",
                        fg_color= hide_color, 
                        text_color= hide_color,
                    )
                except Exception:
                    pass

    def set_tiles_enabled_style(self):
        """解答開始時：すべてのプールタイルを通常表示（操作可能）に戻す"""
        if hasattr(self, 'pool_buttons') and self.pool_buttons:
            for btn in self.pool_buttons:
                try:
                    btn.configure(
                        state="normal",
                        fg_color="white",
                        text_color=TEXT
                    )
                except Exception:
                    pass


    def start_flash_timer(self):
        """ステップ①: カウントダウン開始 (READY... 3, 2, 1)"""
        self.cancel_flash_timer()

        # モード状態の判定
        is_flash = self.flash_mode.get() if hasattr(self, 'flash_mode') else False
        is_listen = self.listening_mode.get() if hasattr(self, 'listening_mode') else False

        # ★通常モード（どちらもOFF）のときは READY 処理を完全にスキップして脱出
        if not (is_flash or is_listen):
            self.flash_step = ""
            self.is_tile_locked = False
            return

        # タイルを一時的にロック
        self.is_tile_locked = True
        self.set_tiles_disabled_style()

        # タイマーステップとカウントの初期化
        self.flash_step = "READY"  # 現在のフェーズ: "READY" または "SHOW"
        self.flash_count = 3

        # 画面表示を READY に設定（FLASH / LISTEN モード時のみ通過）
        if hasattr(self, 'result_label'):
            self.result_label.configure(
                text=f"READY... {self.flash_count}", 
                text_color="#E67E22"
            )

        # 1秒ごとにタイマーを更新
        self.flash_timer_job = self.after(1000, self.update_flash_timer)
   


    def update_flash_timer(self):
        """カウントダウン＆表示時間タイマー処理"""
        self.flash_count -= 1

        if self.flash_count > 0:
            if self.flash_step == "READY":
                if hasattr(self, 'result_label'):
                    self.result_label.configure(
                        text=f"READY... {self.flash_count}", 
                        text_color="#E67E22"
                    )
            self.flash_timer_job = self.after(1000, self.update_flash_timer)
        else:
            if self.flash_step == "READY":
                # --- LISTEN MODE の場合 ---
                if self.listening_mode.get():
                    if hasattr(self, 'result_label') and not self.selected:
                        self.result_label.configure(
                            text="[ LISTEN & RECONSTRUCT! ]", 
                            text_color="#2980B9"
                        )

                    # 音声再生をスタート
                    self.speak_sentence()
                   
                    # 1.5秒後に最初のチェックを行う
                    self.flash_timer_job = self.after(1500, self.check_audio_playing)

                # --- FLASH MODE の場合 ---
                else:
                    self.flash_step = "SHOW"
                    self._update_result()

                    # 単語数に応じた表示秒数を設定
                    word_count = len(getattr(self, 'words', [])) or 5
                    if word_count <= 5:
                        show_sec = 3
                    elif word_count <= 8:
                        show_sec = 4
                    elif word_count <= 10:
                        show_sec = 6
                    elif word_count <= 13:
                        show_sec = 7
                    elif word_count <= 16:
                        show_sec = 8
                    else:
                        show_sec = 10

                    self.flash_count = show_sec
                    self.flash_timer_job = self.after(1000, self.update_flash_timer)

            elif self.flash_step == "SHOW":
                # FLASH MODE: 表示時間が終了したらロック解除
                self.hide_question_text()



    def check_audio_playing(self):
        """LISTEN MODE用: 音声再生中かどうかをチェックして終了時にロック解除"""
        try:
            # pygameで音声が再生中かチェック
            if pygame.mixer.music.get_busy():
                # まだ再生中の場合は、1.5秒（1500ms）待ってから再確認
                self.flash_timer_job = self.after(1500, self.check_audio_playing)
            else:
                # 再生が完全に終わったらタイルのロックを解除して解答開始
                self.hide_question_text()
        except Exception:
            self.hide_question_text()

    

    def hide_question_text(self):
        """最終ステップ: 伏字化して解答スタート"""
        self.flash_timer_job = None
        
        # タイルのロック解除
        self.is_tile_locked = False
        self.set_tiles_enabled_style()

        # 時間計測スタート
        self.round_start_time = time.perf_counter()

        # ① LISTEN MODE の場合
        if self.listening_mode.get():
            if hasattr(self, 'result_label') and not self.selected:
                self.result_label.configure(
                    text="[ LISTEN & RECONSTRUCT! ]", 
                    text_color="#2980B9"
                )
            

        # ② FLASH MODE の場合（表示時間が終わったので伏字にする）
        elif getattr(self.flash_mode, "get", lambda: False)():
            if hasattr(self, 'result_label') and not self.selected:
                self.result_label.configure(
                    text="[ MEMORIZE & RECONSTRUCT! ]", 
                    text_color="#C0392B"
                )

        # ③ 通常モードの場合
        else:
            self._update_result()




    

    def _run_countdown(self, count):
        """1秒ごとに3, 2, 1を表示し、終了後に英文表示タイマーを起動"""
        if count > 0:
            # カウントダウン中の文字表示（大きく目立つ色で表示）
            self.result_label.configure(text=f"READY... {count}", text_color="#E67E22")
            # 1000ms (1秒) 後に次のカウントを実行
            self.cd_timer_job = self.after(1000, self._run_countdown, count - 1)
        else:
            self.cd_timer_job = None
            # カウント終了！画面を更新して本文を表示し、FLASHタイマーをスタート
            
            
            # --- 単語数に応じた表示時間の設定 ---
            word_count = len(getattr(self, 'words', [])) or 5
            if word_count <= 5:
                delay_ms = 3000   # 5語以下：3秒
            elif word_count <= 8:
                delay_ms = 4500   # 6〜8語：4.5秒
            elif word_count <= 10:
                delay_ms = 6000   # 9〜10語：6秒
            elif word_count <= 13:
                delay_ms = 7000   # 11〜13語：7秒
            elif word_count <= 16:
                delay_ms = 8000   # 14〜16語：8秒
            else:
                delay_ms = 10000   # 17語以上：10秒

            self.flash_timer_job = self.after(delay_ms, self.hide_question_text)

            self._update_result()



    


    def cancel_flash_timer(self):
        # カウントダウンタイマーのキャンセル
        if hasattr(self, 'cd_timer_job') and self.cd_timer_job is not None:
            self.after_cancel(self.cd_timer_job)
            self.cd_timer_job = None

        # 5秒タイマーのキャンセル
        if hasattr(self, 'flash_timer_job') and self.flash_timer_job is not None:
            self.after_cancel(self.flash_timer_job)
            self.flash_timer_job = None

        # ★ タイマーキャンセル時はタイルのロックも解除する
        self.is_tile_locked = False

# ★ キャンセル時も確実に通常の操作可能カラーに戻す
        self.set_tiles_enabled_style()

    
    
    def restore_question_text(self):
        """表示の初期化（タイマー解除時用）"""
        pass


    def on_listening_toggle(self):
        """LISTEN MODE スイッチ切り替え時の処理"""
        if self.listening_mode.get():
            # FLASH MODE が ON なら OFF に戻す
            if hasattr(self, 'flash_mode') and self.flash_mode.get():
                self.flash_mode.set(False)
                self.cancel_flash_timer()
            
            # FLASH MODE のスイッチを操作不能にする
            if hasattr(self, 'flash_switch'):
                self.flash_switch.configure(state="disabled")
        else:
            # LISTEN MODE が OFF になったら FLASH MODE スイッチを有効化
            if hasattr(self, 'flash_switch'):
                self.flash_switch.configure(state="normal")

        # スイッチを切り替えたら画面/タイマーを再構築
        if hasattr(self, 'reset_round'):
            self.reset_round()
        elif hasattr(self, '_build_pool'):
            self._build_pool()





    def apply_listening_mode_ui(self):
        """LISTEN MODE 時の表示処理"""
        if hasattr(self, 'result_label') and not getattr(self, "game_has_ended", False):
            # まだ一度も単語を選んでいない場合、または途中操作時に伏字化
            if not self.selected:
                self.result_label.configure(
                    text="[ LISTEN & RECONSTRUCT! ]", 
                    text_color="#2980B9"  # 青系の視覚表示
                )



    def update_progress_ui(self):
      """テキストファイルを見て、画面のラベルを最新の数字に書き換える"""
      # 1. 今までに習得した数を取得して称号を決める
      acquired_cnt = get_acquired_count()
      rank_info = get_rank_info(acquired_cnt)

      # 2. active.txt の残り行数を数える
      remaining_cnt = 0
      if os.path.exists("active.txt"):
        with open("active.txt", "r", encoding="utf-8") as f:
          remaining_cnt = sum(1 for line in f if line.strip())

      # 3. 画面のラベルの文字を書き換える
      self.progress_label.configure(
          text=(
              f"[{rank_info['title']}] Rank {rank_info['rank']}   |   "
              f"Remaining: {remaining_cnt}   |   Acquired: {acquired_cnt}"
          )
      )






if __name__ == "__main__":
    ESBApp().mainloop()